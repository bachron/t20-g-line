...
| 🆕 **HEXATI Titanium Seatpost** (Chris Yeo add-on) | 31.8mm (T20 spec; confirm) | ⏳ **Not published** (searched 25 Sep). For scale, another 31.8mm Brompton Ti post (TIBROMTACK) is listed at 252g (535mm) to 290g (580mm) | **+S$100** added to the frameset bundle | **By far the cheapest Ti post tracked**, about half the WOOdman's S$189. Made for the T20 frame, so fit is not a question. Titanium, so it passes the child-seat clamp rule. ⚠️ **Get the weight and length from Chris Yeo.** If it's under ~280g it beats the WOOdman on weight *and* price. Only available with the T20 frameset, so not an option for Build B |
